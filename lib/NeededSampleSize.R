...
filtCase <- Data.filter(Case*6000, "data", 3, 1)
filtCntrl <- Data.filter(Cntrl*6000, "data", 3, 1)
Case <- filtCase
Cntrl <- filtCntrl

data <- merge.features(Case,Cntrl, 0)
group1 <- data[[1]]
group2 <- data[[2]]
...
## the input schould be two matrices of abundances in percents
## eps is the number that substitutes zeros
...
HMPcompare2gr <- function (gr1, gr2, MC = 1000){
  dataCase <- gr1
  dataCntrl <- gr2
  Nrs.Case <- rowSums(gr1)
  Nrs.Cntrl <- rowSums(gr2)
...
#######################################
# Test data for power.curve############
#######################################
Cntrl <- saliva*200/rowSums(saliva)
Case <- tonsils*200/rowSums(tonsils)
###################################